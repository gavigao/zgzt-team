...
export default function AdminUsers() {
  ...
  const handleToggleRole = async (user) => {
    if (!confirm(`确定将 ${user.username || user.account} ${user.role==='admin'?'降级为队员':'提升为管理员'}？`)) return;
    try {
      ...
      <div className="bg-white rounded-2xl card-shadow overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b text-left text-text-sub text-xs">
            <th className="p-3 pl-4">账号</th><th className="p-3">用户名</th><th className="p-3">邮箱</th><th className="p-3">角色</th><th className="p-3">注册时间</th><th className="p-3 pr-4">操作</th>
          </tr></thead>
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                <td className="p-3 pl-4 font-medium flex items-center gap-1.5">{u.account}{u.role==='admin'&&<Shield size={13} className="text-primary"/>}</td>
                <td className="p-3 text-text-sub">{u.username||'未设置'}</td>
