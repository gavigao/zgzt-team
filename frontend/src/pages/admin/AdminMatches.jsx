...
const EMPTY = { opponent: '', match_date: '', location: '', home_away: 'home', our_score: '', opponent_score: '', summary: '', season_id: '', is_highlighted: false };
...
export default function AdminMatches() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const data = { ...form, our_score: form.our_score!==''?parseInt(form.our_score):null, opponent_score: form.opponent_score!==''?parseInt(form.opponent_score):null, season_id: form.season_id||null, is_highlighted:!!form.is_highlighted };
      ...
      <div className="bg-white rounded-2xl card-shadow overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-gray-100 text-left text-text-sub text-xs">
            <th className="p-3 pl-4">日期</th><th className="p-3">对手</th><th className="p-3">比分</th><th className="p-3">赛季</th><th className="p-3 pr-4">操作</th>
          </tr></thead>
          <tbody>
            {matches.map(m=>(<tr key={m.id} className="border-b border-gray-50 hover:bg-gray-50/50">
              <td className="p-3 pl-4">{m.match_date}</td><td className="p-3 font-medium">{m.opponent}</td><td className="p-3 tabular-nums">{m.our_score??'-'}:{m.opponent_score??'-'}</td><td className="p-3 text-text-sub text-xs">{m.season_name||'-'}</td>
